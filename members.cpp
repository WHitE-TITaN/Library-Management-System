...
int members::registerUser(string userName, string userAddress, long long userMobileNo){
    string rawInput;
...
string members::getUserSummary(int id) {
    auto it = allMembers.find(id);
    if (it == allMembers.end()) return "User not found!";

    Member* user = it->second;
    char buffer[100];
    strftime(buffer, sizeof(buffer), "%d/%m/%Y", &user->subscription.second);
